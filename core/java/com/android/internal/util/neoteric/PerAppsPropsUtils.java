...
public final class PerAppsPropsUtils {
    ...
    public static void setProps(Context context) {
        ...
        try {
            ...
            PropImitationHooks.dlog("PerAppsPropsUtils: Failed to read spoofed apps setting: " + e.getMessage());
            return;
        }
        ...
        try {
            ...
            PropImitationHooks.dlog("PerAppsPropsUtils: Failed to parse custom profiles: " + e.getMessage());
        }
        ...
        for (String app : apps) {
            ...
            if (pkg.equals(packageName)) {
                if (allProps.containsKey(device)) {
                    PropImitationHooks.dlog("PerAppsPropsUtils: Applying profile for: " + packageName + " as " + device);
                    Map<String, Object> props = allProps.get(device);
                    for (Map.Entry<String, Object> prop : props.entrySet()) {
                        PropImitationHooks.setPropValue(prop.getKey(), prop.getValue());
                    }
                }
                break;
            }
